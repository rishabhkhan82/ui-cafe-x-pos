...
export class OrderTypeMasterComponent implements OnInit {
  items: OrderType[] = [];
  selectedItem: OrderType | null = null;
  editingItem: OrderType | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getOrderTypes(params).subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load order types. Please try again.';
  ...
  selectItem(item: OrderType): void {
    this.selectedItem = item;
  }

  showItemForm(item?: OrderType): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createOrderType(itemRequest).subscribe({
      next: (response) => {
        console.log('Order type created successfully:', response);
        this.notificationService.success('Item Created', 'The order type has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating order type:', error);
        const apiMessage = error.error?.message || 'Failed to create order type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateOrderType(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Order type updated successfully:', response);
        this.notificationService.success('Item Updated', 'The order type has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to update order type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: OrderType): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteOrderType(item.id).subscribe({
        next: () => {
          console.log('Order type deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete order type. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: OrderType, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateOrderType(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Order type status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating order type status:', error);
        const apiMessage = error.error?.message || 'Failed to update order type status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
