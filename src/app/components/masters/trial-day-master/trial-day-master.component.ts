...
import { TrialDay } from '../../../interfaces';

@Component({
  selector: 'app-trial-day-master',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './trial-day-master.component.html'
})
export class TrialDayMasterComponent implements OnInit {
  items: TrialDay[] = [];
  selectedItem: TrialDay | null = null;
  editingItem: TrialDay | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getTrialDays(params).subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load trial days. Please try again.';
  ...
  selectItem(item: TrialDay): void {
    this.selectedItem = item;
  }

  showItemForm(item?: TrialDay): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createTrialDay(itemRequest).subscribe({
      next: (response) => {
        console.log('Trial day created successfully:', response);
        this.notificationService.success('Item Created', 'The trial day has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating trial day:', error);
        const apiMessage = error.error?.message || 'Failed to create trial day. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateTrialDay(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Trial day updated successfully:', response);
        this.notificationService.success('Item Updated', 'The trial day has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating trial day:', error);
        const apiMessage = error.error?.message || 'Failed to update trial day. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: TrialDay): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteTrialDay(item.id).subscribe({
        next: () => {
          console.log('Trial day deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete trial day. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: TrialDay, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateTrialDay(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Trial day status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating trial day status:', error);
        const apiMessage = error.error?.message || 'Failed to update trial day status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
