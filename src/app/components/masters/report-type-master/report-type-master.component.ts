...
export class ReportTypeMasterComponent implements OnInit {
  items: ReportType[] = [];
  selectedItem: ReportType | null = null;
  editingItem: ReportType | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getReportTypes(params).subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load report types. Please try again.';
  ...
  selectItem(item: ReportType): void {
    this.selectedItem = item;
  }

  showItemForm(item?: ReportType): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createReportType(itemRequest).subscribe({
      next: (response) => {
        console.log('Report type created successfully:', response);
        this.notificationService.success('Item Created', 'The report type has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating report type:', error);
        const apiMessage = error.error?.message || 'Failed to create report type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateReportType(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Report type updated successfully:', response);
        this.notificationService.success('Item Updated', 'The report type has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to update report type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: ReportType): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteReportType(item.id).subscribe({
        next: () => {
          console.log('Report type deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete report type. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: ReportType, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateReportType(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Report type status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating report type status:', error);
        const apiMessage = error.error?.message || 'Failed to update report type status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
