...
export class StateMasterComponent implements OnInit {
  items: State[] = [];
  selectedItem: State | null = null;
  editingItem: State | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getStates(params).subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load states. Please try again.';
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createState(itemRequest).subscribe({
      next: (response) => {
        console.log('State created successfully:', response);
        this.notificationService.success('Item Created', 'The state has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating state:', error);
        const apiMessage = error.error?.message || 'Failed to create state. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateState(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('State updated successfully:', response);
        this.notificationService.success('Item Updated', 'The state has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to update state. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: State): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteState(item.id).subscribe({
        next: () => {
          console.log('State deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete state. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: State, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateState(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('State status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating state status:', error);
        const apiMessage = error.error?.message || 'Failed to update state status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
