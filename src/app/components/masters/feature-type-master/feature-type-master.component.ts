...
@Component({
  selector: 'app-feature-type-master',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './feature-type-master.component.html'
})
export class FeatureTypeMasterComponent implements OnInit {
  items: FeatureType[] = [];
  selectedItem: FeatureType | null = null;
  editingItem: FeatureType | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getFeatureTypes(params).subscribe({
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load feature types. Please try again.';
  ...
  selectItem(item: FeatureType): void {
    this.selectedItem = item;
  }

  showItemForm(item?: FeatureType): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createFeatureType(itemRequest).subscribe({
      next: (response) => {
        console.log('Feature type created successfully:', response);
        this.notificationService.success('Item Created', 'The feature type has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating feature type:', error);
        const apiMessage = error.error?.message || 'Failed to create feature type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateFeatureType(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Feature type updated successfully:', response);
        this.notificationService.success('Item Updated', 'The feature type has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to update feature type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: FeatureType): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteFeatureType(item.id).subscribe({
        next: () => {
          console.log('Feature type deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete feature type. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: FeatureType, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateFeatureType(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Feature type status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating feature type status:', error);
        const apiMessage = error.error?.message || 'Failed to update feature type status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
