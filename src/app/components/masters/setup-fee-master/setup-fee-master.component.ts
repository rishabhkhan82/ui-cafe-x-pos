...
import { SetupFee } from '../../../interfaces';

@Component({
  selector: 'app-setup-fee-master',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './setup-fee-master.component.html'
})
export class SetupFeeMasterComponent implements OnInit {
  items: SetupFee[] = [];
  selectedItem: SetupFee | null = null;
  editingItem: SetupFee | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getSetupFees(params).subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load setup fees. Please try again.';
  ...
  selectItem(item: SetupFee): void {
    this.selectedItem = item;
  }

  showItemForm(item?: SetupFee): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createSetupFee(itemRequest).subscribe({
      next: (response) => {
        console.log('Setup fee created successfully:', response);
        this.notificationService.success('Item Created', 'The setup fee has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating setup fee:', error);
        const apiMessage = error.error?.message || 'Failed to create setup fee. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateSetupFee(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Setup fee updated successfully:', response);
        this.notificationService.success('Item Updated', 'The setup fee has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to update setup fee. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: SetupFee): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteSetupFee(item.id).subscribe({
        next: () => {
          console.log('Setup fee deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete setup fee. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: SetupFee, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateSetupFee(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Setup fee status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating setup fee status:', error);
        const apiMessage = error.error?.message || 'Failed to update setup fee status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
