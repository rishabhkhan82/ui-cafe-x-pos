...
import { WasteType } from '../../../interfaces';
...
export class WasteTypeMasterComponent implements OnInit {
  items: WasteType[] = [];
  selectedItem: WasteType | null = null;
  editingItem: WasteType | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getWasteTypes(params).subscribe({
      next: (response: any) => {
      ...
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to load waste types. Please try again.';
  ...
  selectItem(item: WasteType): void {
    this.selectedItem = item;
  }

  showItemForm(item?: WasteType): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createWasteType(itemRequest).subscribe({
      next: (response) => {
        console.log('Waste type created successfully:', response);
        this.notificationService.success('Item Created', 'The waste type has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
        console.error('Error creating waste type:', error);
        const apiMessage = error.error?.message || 'Failed to create waste type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateWasteType(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Waste type updated successfully:', response);
        this.notificationService.success('Item Updated', 'The waste type has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
      error: (error) => {
        ...
        const apiMessage = error.error?.message || 'Failed to update waste type. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
  ...
  async deleteItem(item: WasteType): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteWasteType(item.id).subscribe({
        next: () => {
          console.log('Waste type deleted successfully:', item.id);
        ...
        error: (error) => {
          ...
          const apiMessage = error.error?.message || 'Failed to delete waste type. Please try again.';
          this.errorMessage = apiMessage;
          this.loadingService.hide();
        }
      });
  ...
  updateItemStatus(item: WasteType, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateWasteType(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Waste type status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating waste type status:', error);
        const apiMessage = error.error?.message || 'Failed to update waste type status. Please try again.';
        this.errorMessage = apiMessage;
        this.loadingService.hide();
      }
    });
