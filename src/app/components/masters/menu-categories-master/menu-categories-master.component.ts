...
@Component({
  selector: 'app-menu-categories-master',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './menu-categories-master.component.html'
})
export class MenuCategoriesMasterComponent implements OnInit {
  items: MenuCategory[] = [];
  selectedItem: MenuCategory | null = null;
  editingItem: MenuCategory | null = null;
  searchTerm = '';
  statusFilter = 'all';
  showAddForm = false;
  errorMessage = '';
  ...
  loadItems(): void {
    ...
    this.crudService.getMenuCategories(params).subscribe({
      next: (response: any) => {
  ...
  selectItem(item: MenuCategory): void {
    this.selectedItem = item;
  }

  showItemForm(item?: MenuCategory): void {
  ...
  private onSaveForm(): void {
    ...
    this.crudService.createMenuCategory(itemRequest).subscribe({
      next: (response) => {
        console.log('Menu category created successfully:', response);
        this.notificationService.success('Item Created', 'The menu category has been successfully created.');
        this.resetForm();
        this.loadItems();
      },
      error: (error) => {
  ...
  private onUpdateForm(): void {
    ...
    this.crudService.updateMenuCategory(this.editingItem!.id, itemRequest).subscribe({
      next: (response) => {
        console.log('Menu category updated successfully:', response);
        this.notificationService.success('Item Updated', 'The menu category has been successfully updated.');
        this.resetForm();
        this.loadItems();
        this.loadingService.hide();
      },
  ...
  async deleteItem(item: MenuCategory): Promise<void> {
    ...
    if (confirmed) {
      ...
      this.crudService.deleteMenuCategory(item.id).subscribe({
        next: () => {
          console.log('Menu category deleted successfully:', item.id);
  ...
  updateItemStatus(item: MenuCategory, newStatus: boolean): void {
    this.loadingService.show();
    const updatedItem = { ...item, is_active: newStatus, updated_at: new Date() };

    this.crudService.updateMenuCategory(item.id, updatedItem).subscribe({
      next: (response) => {
        console.log('Menu category status updated successfully:', response);
        item.is_active = newStatus;
        this.loadingService.hide();
      },
      error: (error) => {
        console.error('Error updating menu category status:', error);
